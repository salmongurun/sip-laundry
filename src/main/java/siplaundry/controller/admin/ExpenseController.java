...
import siplaundry.view.admin.components.column.ExpenseColumn;
...
public class ExpenseController {
    ...
    void searchAction(KeyEvent event){
        List<ExpenseEntity> exp = expRepo.searchTable(
            "users.fullname", 
    ...
    void sortAction(){
        ...
        if(CB_column.getValue() != null){
            ...
            if(CB_column.getValue().equals("Katagori Pengeluaran")) column = " name";
        }
       

        List<ExpenseEntity> exp = expRepo.sortTable(
            "users.fullname", 
            " JOIN users ON expense.user_id = users.user_id", 
            this.searchableValues(), 
            column, 
